---
title: "Finland Parliamentary Elections 2023"
...
color_dat <- tibble(
  colors = c("#006b99", "#006845", "#f00a64", "#ffde55", "#ffb612", "#3aad2e", "#f54b4b", "#1d7fb9", "#c41200", "#ffdd93"),
  party = c("KOK", "VIH", "VAS", "PER", "MUUT", "KES", "SDP", "KD", "KOR", "RKP"))
...
fi_advertisers <- read_csv("data/fi_advertisers.csv")
hc_data <-  more_data %>% 
  # mutate(advertiser_id = as.character(advertiser_id)) %>% 
...
add_ribbons("KOK", "#006b99") %>%
add_ribbons("VIH", "#006845") %>%
add_ribbons("VAS", "#f00a64") %>%
add_ribbons("PER", "#ffde55") %>%
add_ribbons("MUUT", "#ffb612") %>%
add_ribbons("KES", "#3aad2e") %>%
add_ribbons("SDP", "#f54b4b") %>%
add_ribbons("KD", "#1d7fb9") %>%
add_ribbons("KOR", "#c41200") %>%
add_ribbons("RKP", "#ffdd93")
...
gg <- coltototal30 %>% 
  ...
  mutate(target = case_when(
    ...
    target == "countries" ~ "GEOGRAPHY: Finland",
...
the_order <- contested_dat %>% 
  filter(party == "VIH") %>%   arrange(desc(perc)) %>% 
  pull(value) %>% 
  unique()
...
region_dat <- election_dat30 %>%
  ...
  ungroup() %>%
  rename(region = value) %>%
  mutate(region = str_remove_all(region, ", Montenegro")) 
...
x %>% 
  ...
  filter(!is_exclusion) %>% 
    ...
    filter(str_detect(value, "Wine|Master of", negate = T)) %>% 
  mutate(total_spend = total_spend_formatted) %>%
  mutate(total_spend = ifelse(total_spend == 50, 1, total_spend)) %>% 
  # filter(coalition == "Coalizione di centro-destra") %>%  
  # filter(total_spend_pct != 0) %>% 
...
```{r, fig.width= 11, fig.height=11, dpi = 300}
...
```{r, fig.width=14, fig.height=11, dpi=300}
